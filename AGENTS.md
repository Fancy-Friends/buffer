...
npm run provider -- buffer            # regenerate this repo
npm run provider -- buffer --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/buffer-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/buffer-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/buffer-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-buffer` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/buffer/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/buffer` |
| npm | `@particle-academy/buffer-ui`, `@particle-academy/buffer-js` |
| Packagist | `particle-academy/buffer-php` |
| PyPI | `fancy-buffer` |
...
**Default to plain HTTP.** A Buffer SDK is third-party code and is subject to
