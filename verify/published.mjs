/*
 * Buffer — the published npm packages.
 *
...
import { bufferFaker } from "@particle-academy/buffer-js";
import { BUFFER_KINDS } from "@particle-academy/buffer-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/buffer-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = bufferFaker(operation, fakeRequest("buffer", operation, config));
...
assert.equal(BUFFER_KINDS.length, 1);
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/buffer-ui"}`);
